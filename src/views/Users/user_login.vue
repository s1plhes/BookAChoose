<template>
  ...
  <section class="grid lg:grid-cols-2 sm:grid-cols-1 mx-auto my-8 max-w-6xl min-h-screen">
    <!-- Background Image -->
    <div class="w-full max-w-screen-md p-8 justify-self-center">
      <Logo class="mx-auto" colour="#FEFFFA" width="300px" />
      <div class="text-slate-100 text-center">Share with us</div>
    </div>
    <!-- Login Form -->
    <div class="px-6">
      <div class="w-full  space-y-4">
        ...
        <form class="space-y-4 " @submit.prevent="login">
          ...
          <div>
            ...
            <input type="email" id="email" v-model="email" placeholder="Enter Email Address" class="input" autofocus
              autocomplete="on" required />
          </div>
          <!-- Password Field -->
          <div>
            <label for="password" class="block text-slate-200 font-semibold">Password</label>
            <input type="password" id="password" v-model="password" placeholder="Enter Password" minlength="8"
  ...
  try {
    ...
    if (response.status === 200 && response.data.accessToken) {
      ...
      if (decodedJWT && decodedJWT.userId) {
        const userResponse = await axios.get(`${import.meta.env.VITE_API}/users/${decodedJWT.userId}`);

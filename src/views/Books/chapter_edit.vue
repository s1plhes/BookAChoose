...
const fetchChapterData = async () => {
    try {
        const response = await axios.get(`${import.meta.env.VITE_API}/book/${bookId}/chapter/${chapterId}`)
...
const saveChapter = async () => {
    try {
        await axios.put(`${import.meta.env.VITE_API}/chapter/${chapterId}`,
            {
                //send data
                title: title.value,
    ...
    <div class="container max-w-8xl mx-auto p-3 min-h-fit h-full">
        ...
        <form @submit.prevent="saveChapter" class="space-y-6 mt-3">
            <div class="form-group">
                <label for="chapter-title" class="block text-lg font-medium text-gray-100">Chapter Title</label>
                <input type="text" id="chapter-title" v-model="title" required class="input"
                    placeholder="Enter the chapter title">
            </div>

            <div class="form-group">
                <label for="chapter-content" class="block text-lg font-medium text-gray-100">Chapter Content</label>
                <div class="mt-1 bg-slate-100 rounded-md">
                    <trix-editor class="bg-slate-950 text-slate-100 h-fit min-h-[300px] w-full" id="chapter-content"
                        @trix-change="updateChapterContent"></trix-editor>
                </div>
            </div>

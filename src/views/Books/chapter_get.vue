...
const fetchABook = async () => {
    try {
        const { data } = await axios.get(`http://localhost:3000/api/book/${bookId}/chapter/${chapterId}`);
        bookData.value = data;
...
const getLikes = async () => {
    try {
        const { data } = await axios.get(`http://localhost:3000/api/count/likes/${bookId}/${chapterId}`);
...
<template>
    ...
    <div class="container mx-auto p-4 text-white mb-6 bg-slate-600/10 backdrop-blur-sm rounded-lg">
        ...
        <div class="space-y-3 mb-6">
            <!-- Tools toolbar -->
            <div v-if="user && bookData"
                class="p-2 inline-flex items-center backdrop-blur-sm bg-slate-900/40 rounded space-x-5">
                <div v-if="bookData.chapter_editor === user.name" class="space-x-4">

                    <Btn variant="primary" :href="`/book/${bookData.book_id}/chapter/${id}/edit`"> Edit chapter </Btn>
                </div>
                <!-- LIKE SYSTEM -->
                <div>
                    <LikeBtn :userId :bookId :chapterId />
                </div>
                <div><i class="fa fa-heart"></i> {{ likesCount }} likes
                </div>
                <div>
                    <ViewIcon /> {{ bookData.views }}
                </div>
                <div> <span>Font Size:</span>
                    <button type="button" @click="decreaseFontSize" class="bg-transparent px-2 py-1 rounded"><i
                            class="fa-solid fa-circle-minus"></i></button>
                    <span>{{ fontSize }}px</span>
                    <button type="button" @click="increaseFontSize" class="bg-transparent px-2 py-1 rounded"><i
        ...
        <div v-if="bookData">
            <h2 class="text-3xl font-semibold mb-4">{{ bookData.title }}</h2>
        </div>

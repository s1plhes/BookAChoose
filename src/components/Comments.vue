...
onMounted(async () => {
    try {
        const response = await axios.get(`http://localhost:3000/api/comments/${bookId}/${chapterId}`);
...
<template>
    <div class="comment-section p-4 bg-slate-900/30 backdrop-blur-md rounded-lg shadow-md">
        ...
        <div class="mt-6">
            ...
            <div v-if="replyTo" class="mb-2 p-2 bg-blue-100 rounded-lg text-blue-800">
                Replying to: {{ replyTo.name }} <button @click="replyTo = null"
                    class="ml-2 text-red-500">Cancel</button>
            </div>

...
<template>
  <div class="relative inline-block text-left z-20">
    <div class="inline">
      ...
      >
        <div class="space-x-5 inline-flex items-center">
          <img :src="`${Cookies.get('avatar')}`" class="w-8 h-8 rounded-full mr-2 object-center" />
          {{ placeholder }}
          <svg
    ...
    <div
      v-if="isOpen"
      class="origin-top-right absolute left-0 mt-2 w-56 rounded-md shadow-lg backdrop-filter backdrop-blur bg-slate-900/80 focus:outline-none hover:bg-slate-900/80 ring-1 ring-black ring-opacity-5 z-40"
      role="menu"
